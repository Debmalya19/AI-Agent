...
def create_ticket_tool(query: str, user_id: Optional[int] = None) -> str:
    """
    Create a support ticket for user issues. Use this when a user needs
    technical support, billing assistance, or any issue that requires human intervention.
    
    The input should be a description of the issue and user information if available.
    """
    db = SessionLocal()
    try:
        ...
        if any(word in query.lower() for word in ['error', 'bug', 'crash', 'not working']):
            category = TicketCategory.BUG_REPORT
        elif any(word in query.lower() for word in ['bill', 'payment', 'charge', 'refund', 'cost']):
            category = TicketCategory.BILLING
        elif any(word in query.lower() for word in ['feature', 'suggestion', 'improve', 'add']):
            category = TicketCategory.FEATURE_REQUEST
        elif any(word in query.lower() for word in ['technical', 'setup', 'configure', 'install']):
            category = TicketCategory.TECHNICAL
            
        # Determine priority
        priority = TicketPriority.MEDIUM
        if any(word in query.lower() for word in ['urgent', 'emergency', 'critical', 'serious']):
            priority = TicketPriority.HIGH
        elif any(word in query.lower() for word in ['blocked', 'cannot access', 'down', 'outage']):
            priority = TicketPriority.CRITICAL
            
        # Create a descriptive title
        title = f"Support Request: {query[:50]}..." if len(query) > 50 else f"Support Request: {query}"
        ...
        tags = ["support", "customer-request", category.value, priority.value]
        
        ticket = service.create_ticket(
            title=title,
            description=f"Issue Description: {query}\n\nCategory: {category.value}\nPriority: {priority.value}",
            user_id=user_id,
            priority=priority,
            category=category,
            tags=tags,
        ...
        # Create a professional and concise response
        response = f"""Support ticket {ticket.id} has been created to address your issue.

Priority: {priority.value.capitalize()}
Category: {category.value.replace('_', ' ').capitalize()}
Status: {ticket.status.value.capitalize()}

Our technical team will review your case and contact you soon. You can reference ticket #{ticket.id} in future communications."""
